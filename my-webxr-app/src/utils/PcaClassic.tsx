 ...
 *    2: Calculate the k-th right singular vectors by performing singular value decomposition on
 *    dataset.
...
export function getRightSingularVectors(datasetMatrix: Matrix, kComponents: number): Matrix {
  ...
  const { rightSingularVectors } = svd;
  return rightSingularVectors.subMatrix(0, rightSingularVectors.rows - 1, 0, kComponents - 1);
}

/**
 * This function performs PCA on a data set using the Singular Value Decomposition (SVD) method,
 * also known as Classic PCA.
 *
 * It first standardizes the dataset, ensuring all features have a mean of 0 and a standard
 * deviation of 1. It then computes the right singular vectors of the standardized dataset, which
 * serve as the principal components. The dataset is then transformed by projecting it onto the
 * principal components, and then returned.
 *
 * @param {Matrix} datasetMatrix - The dataset to perform PCA on, represented as a matrix where each
 * row is an observation and each column is a feature.
 * @param {number} kComponents - The number of principal components to retain.
 * @returns {Matrix} The PCA-transformed dataset, with only the first kComponents principal
 * components. If an error occurs during the PCA computation, an empty matrix is returned.
 * @throws {Error} If kComponents is less than 1 or greater than the number of features in the
 * dataset.
 */
export function computeClassicPCA(datasetMatrix: Matrix, kComponents: number): Matrix {
  assert(kComponents > 0 && kComponents <= datasetMatrix.columns, `Invalid kComponents value: ${kComponents}`);
  try {
    const rightSingularVectors = getRightSingularVectors(
      standardizeDataset(datasetMatrix),
      kComponents,
    );
