 ...
 *    4: Pick k eigenvalues and form a matrix of eigenvectors. Then take product of dataset and
 *    eigenvectors to form PCA matrix.
 * /
 ...
 * . Then calculates the covariance matrix. The eigenvectors of the covariance matrix are computed,
 * which are then used to transform the dataset. The transformed dataset is then returned, with only
 * the first kComponents principal components.
 *
...
export function computeCovariancePCA(datasetMatrix: Matrix, kComponents: number): Matrix {
  ...
  try {
    const covarianceMatrix = calculateCovarianceMatrix(standardizeDataset(datasetMatrix));
