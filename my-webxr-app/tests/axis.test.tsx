...
describe('Axis Tests', () => {
    test("Create 3D axis with random values", async () => {
        const renderer = await ReactThreeTestRenderer.create(
            <XR>
                <Axis minValue={0} maxValue={10} scaleFactor={1} labelOffset={0.1} startX={0} startY={0.82}
                      startZ={-0.15}
                      endPoint={1}
                      radius={0.002}></Axis>
            </XR>
        ...
        expect(renderer.scene.children[1].children[0].instance.position).toEqual(new Vector3(0, 0.82, -0.15))
    })

    // test("Create single axis values 0-10 and see if increment is 1", async () => {
    //     const renderer = await ReactThreeTestRenderer.create(
    //         <XR>
    //             <SingleAxis startX={0} startY={0.82} startZ={-0.15} endX={1} endY={0.82} endZ={-0.15} radius={0.002}
    //                         labelOffset={0.1}
    //                         scaleFactor={1}
    //                         minValue={0}
